...
def main() -> None:
    ...
    if bundle_path is None:
        bundle_path = find_latest_bundle(
            bundle_root=bundle_root, generator=args.generator
        )
        if bundle_path is None:
            raise SystemExit(
    ...
    if bundle.spec.generator != args.generator:
        raise SystemExit(
            f"Bundle metadata mismatch. bundle={bundle.paths.root} generator={bundle.spec.generator}"
        )
