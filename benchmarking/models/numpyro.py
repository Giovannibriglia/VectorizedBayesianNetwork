...
def _sorted_nodes(dag) -> list[Any]:
    ...
    # Fallback on deterministic lexical ordering if the graph isn't sortable.
    try:
        return sorted(nodes)
    except TypeError:
        ...
        for node in self._topo:
            ...
            if meta.get("type") == "continuous":
                raise NotImplementedError(
                    "numpyro backend currently supports discrete nodes only"
                )
            states = list(meta.get("states") or [])
            if not states:
                raise ValueError(
                    "numpyro backend requires explicit discrete states for each node. "
