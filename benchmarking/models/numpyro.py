...
def _package_version() -> str | None:
    try:
        return importlib_metadata.version("numpyro")
    except Exception:
        return None
...
class NumpyroBenchmarkModel(BaseBenchmarkModel):
    name = "numpyro"
    family = "numpyro"
    ...
    def fit(
        ...
        if alpha <= 0:
            raise ValueError("numpyro learning alpha must be > 0")
        min_std = float(learning_kwargs.get("min_std", 1e-3))
        if min_std <= 0:
            raise ValueError("numpyro learning min_std must be > 0")
        ...
        if int(len(data_df.index)) <= 0:
            raise ValueError("Cannot fit numpyro backend with empty data")
        ...
        for node in self._topo:
            ...
            if node_type == "discrete":
                if not states:
                    raise ValueError(
                        "numpyro discrete nodes require explicit states. "
        ...
        for node in self._topo:
            ...
            if self._is_discrete_node(node) and c_par:
                raise NotImplementedError(
                    "numpyro backend supports CLG only when discrete nodes have "
    ...
    def _infer_distribution(
        ...
        if inference_name not in supported:
            raise ValueError(
                f"Unsupported numpyro inference method '{inference_name}'. "
        ...
        for sample_idx in range(int(n_samples)):
            ...
            for node in self._topo:
                ...
                if self._is_discrete_node(node):
                    ...
                    probs_jax = self._device_put(
                        self._jnp.asarray(probs, dtype=self._jnp.float32)
                    )
                    ...
                    if node in local_evidence:
                        ...
                        lp = float(
                            dist.log_prob(
                                self._device_put(
                                    self._jnp.asarray(int(val), dtype=np.int32)
                                )
                            )
                        )
                        if math.isfinite(lp):
                        ...
                        rng_key, subkey = self._jax_random.split(rng_key)
                        sampled = int(dist.sample(subkey))
                ...
                mean_jax = self._device_put(
                    self._jnp.asarray(float(mean), dtype=self._jnp.float32)
                )
                std_jax = self._device_put(
                    self._jnp.asarray(max(float(std), 1e-8), dtype=self._jnp.float32)
                )
                dist = self._dist.Normal(loc=mean_jax, scale=std_jax)
                if node in local_evidence:
                    val = self._coerce_continuous_value(node, local_evidence[node])
                    lp = float(
                        dist.log_prob(
                            self._device_put(
                                self._jnp.asarray(float(val), dtype=self._jnp.float32)
                            )
                        )
                    )
                    if math.isfinite(lp):
                    ...
                    rng_key, subkey = self._jax_random.split(rng_key)
                    sampled = float(dist.sample(subkey))
                    assignment[node] = sampled
