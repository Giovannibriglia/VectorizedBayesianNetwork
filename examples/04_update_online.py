...
def _parse_args():
    parser = argparse.ArgumentParser(description="Online update example.")
...
def main():
    ...
    seed_all(args.seed)
    device = auto_device()
    print_env_header("04_update_online", device)

    df = make_df(n=1000, seed=args.seed)
    g = nx.DiGraph()
    ...
    new_df = make_df(500, seed=args.seed + 1)
    vbn.update(
