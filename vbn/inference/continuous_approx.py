...
class ContinuousApproxInference:
    def __init__(
        self,
        meta: BNMeta,
    ...
    def posterior(
        ...
        weights = torch.ones(
            (1 if B is None else B, S), device=self.device, dtype=self.dtype
        )
        ...
        def cont_cpd(
            ...
            if use_mlp:
                ...
                mean = mean.reshape((1 if B is None else B), S)
                var = torch.exp(logvar).reshape((1 if B is None else B), S)
                return mean, var
            elif have_lg:
                ...
                if len(par_idx) == 0:
                    mean = lp.lg.b[i].expand((1 if B is None else B), S)
                else:
        ...
        for node in self.meta.order:
            ...
            if t == "discrete":
                if node in do:
                    ...
                    samples_disc[node] = x.expand(-1 if B is not None else 1, S)
                    continue
                if node in e_disc:
                    x = (
                ...
                # build parent assignment (discrete parents as ints; continuous as floats)
                pa_assign = {}
                ...
                if node in do:
                    ...
                    samples_cont[node] = xv.expand(-1 if B is not None else 1, S)
                    # no weighting for do
                elif node in e_cont:
                    xv = (
                    ...
                    xv = xv.expand(-1 if B is not None else 1, S)
